...
async function aiGenerateSimpleHtml(payload: any, openaiApiKey: string): Promise<string> {
  const systemPrompt = `You are an expert at summarizing and presenting information. Given a JSON object representing an ad (including fields like adContext, targetURL, etc), generate a single line of plain text summarizing the main value or offer, followed by a period, then a single anchor tag (using the targetURL field as the href) labeled 'Learn more' and including a clickout icon (Unicode: \u2197) after the text. The output should be minimal HTML: just the text, a period, and the anchor tag, nothing else. Example: "Headline - summary. <a href=\"URL\">Learn more ↗</a>"`;
